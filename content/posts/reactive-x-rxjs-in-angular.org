...
Additionally, to see what affects the value of ~isLoading~, you have to read pretty much the *entire* component or search for its references. If you want to change the behavior of ~isLoading~, you'll have to comb through the component and decide whether or not to update the relevant lines.
