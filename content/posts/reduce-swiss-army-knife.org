...
There's a little Array method in JavaScript that I've seen a lot of developers overlook. I suspect because it's abstract and generic, so doesn't lend itself to any obvious uses. But it turns out that's key to a great programming Swiss Army knife: you can do a lot more with it than you might realise.[fn:1]
